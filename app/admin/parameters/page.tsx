  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8 font-sans selection:bg-brand-cyan/20">
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        {/* HLAVIČKA */}
        <header className="flex flex-col md:flex-row justify-between items-center bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm gap-6">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Aktuální hodnoty firemních metrik</p>
            </div>
          </div>
        ...
        <form action={updateCompanyParameters} className="space-y-8">
          ...
          <section className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm relative overflow-hidden">
            ...
            <p className="text-gray-400 text-xs mb-8">Výsledek hospodaření firmy před odpisy, úroky a daněmi.</p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-brand-cyan/70 uppercase tracking-widest block ml-1">Aktuální EBITDA (CZK)</label>
                <input name="currentEbitda" type="number" step="1" defaultValue={p?.currentEbitda ?? 0}
                  className="w-full bg-gray-50 rounded-2xl px-6 py-4 font-black border-2 border-gray-200 focus:border-brand-cyan outline-none transition-all text-gray-900 shadow-inner text-lg" />
                {p && <p className="text-[10px] text-gray-400 ml-2">Nyní: {fmt(p.currentEbitda)} CZK</p>}
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-brand-cyan/70 uppercase tracking-widest block ml-1">Cílová EBITDA (CZK)</label>
                <input name="targetEbitda" type="number" step="1" defaultValue={p?.targetEbitda ?? 0}
                  className="w-full bg-gray-50 rounded-2xl px-6 py-4 font-black border-2 border-gray-200 focus:border-brand-cyan outline-none transition-all text-gray-900 shadow-inner text-lg" />
                {p && <p className="text-[10px] text-gray-400 ml-2">Cíl: {fmt(p.targetEbitda)} CZK</p>}
              </div>
            </div>
          ...
          <section className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm relative overflow-hidden">
            ...
            <p className="text-gray-400 text-xs mb-8">Parametrizovatelná firemní metrika — např. obrat, ARR, zákaznická báze nebo jiný strategický ukazatel.</p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-brand-pink/70 uppercase tracking-widest block ml-1">Aktuální HORIZONT</label>
                <input name="currentHorizont" type="number" step="0.01" defaultValue={p?.currentHorizont ?? 0}
                  className="w-full bg-gray-50 rounded-2xl px-6 py-4 font-black border-2 border-gray-200 focus:border-brand-pink outline-none transition-all text-gray-900 shadow-inner text-lg" />
                {p && <p className="text-[10px] text-gray-400 ml-2">Nyní: {fmt(p.currentHorizont)}</p>}
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-brand-pink/70 uppercase tracking-widest block ml-1">Cílový HORIZONT</label>
                <input name="targetHorizont" type="number" step="0.01" defaultValue={p?.targetHorizont ?? 0}
                  className="w-full bg-gray-50 rounded-2xl px-6 py-4 font-black border-2 border-gray-200 focus:border-brand-pink outline-none transition-all text-gray-900 shadow-inner text-lg" />
                {p && <p className="text-[10px] text-gray-400 ml-2">Cíl: {fmt(p.targetHorizont)}</p>}
              </div>
            </div>
          ...
          <section className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm relative overflow-hidden">
            ...
            <p className="text-gray-400 text-xs mb-8">Aktuální tržní multiplier pro výpočet hodnoty firmy (EBITDA × Multiplier). Mění se na základě strategických výsledků a tržního benchmarku.</p>

            <div className="max-w-xs space-y-2">
              <label className="text-[10px] font-black text-brand-green/70 uppercase tracking-widest block ml-1">Aktuální Multiplier</label>
              <input name="currentMultiplier" type="number" step="0.1" defaultValue={p?.currentMultiplier ?? 6.0}
                className="w-full bg-gray-50 rounded-2xl px-6 py-4 font-black border-2 border-gray-200 focus:border-brand-green outline-none transition-all text-brand-green shadow-inner text-2xl" />
              {p && (
          ...
          <button type="submit" className="w-full bg-brand-cyan text-brand-navy py-5 rounded-2xl font-black hover:bg-brand-pink hover:text-white transition-all shadow-sm active:scale-95 uppercase tracking-[0.2em] text-sm">
            Uložit parametry
          </button>
        </form>
      </div>
